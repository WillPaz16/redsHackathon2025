...
allPlayersDataFiltered <- allPlayersData %>%
  dplyr::select(-weight, -height, -birthYear, -debutYear, -birthCountry, -bats, -throws) %>% 
...
```{r}
test_size <- 1030
train_index <- sample(nrow(allPlayersDataFiltered), test_size)
train_data <- allPlayersDataFiltered[train_index, ]
test_data <- allPlayersDataFiltered[-train_index, ]

train_data_filtered <- train_data %>% dplyr::select(-PLAYER_ID)
test_data_filtered <- test_data %>% dplyr::select(-PLAYER_ID)
...
reg1 <- lm(AVG_PLAYING_TIME ~ ., data = train_data_filtered)
...
stepwiseBIC <- step(null, 
                    ...
                    direction = "both", 
                    k = log(nrow(train_data_filtered)))  # BIC penalty
...
backwardAICStats <- extract_stats(backwardAIC, train_data_filtered, train_control)
stepwiseBICStats <- extract_stats(stepwiseBIC, train_data_filtered, train_control)
...
```{r}
new_formula <- update(stepwiseBIC, . ~ . -total_field_out_batted )
...
predict(stepwiseBIC_updated)
submission1 <- data.frame(
  PLAYER_ID = allPlayersDataFiltered$PLAYER_ID,
  PLAYING_TIME = predict(stepwiseBIC_updated)
) %>% 
...
```{r}
obs <- allPlayersDataFiltered$AVG_PLAYING_TIME
pred <- predict(stepwiseBIC_updated, test_data_filtered)
RMSE <- RMSE(pred, obs, na.rm=TRUE)
RMSE
```
