  ...
  output$batted_ball_table <- renderDataTable({
    ...
    table <- table %>%
      ...
      dplyr::summarize(
        ...
        'Out %' = round(sum(PlayResult %in% c("Strikeout", "Out", "Sacrifice", "FieldersChoice")/n()*100), 1),
        'GB %' = round(sum(TaggedHitType == "GroundBall", na.rm = TRUE) / sum(PitchCall == "InPlay", na.rm = TRUE) * 100, 1),
        'FB %' = round(sum(TaggedHitType %in% c("FlyBall", "Popup"), na.rm = TRUE) / sum(PitchCall == "InPlay", na.rm = TRUE) * 100, 1),
    ...
    table2 <- table2 %>%
      ...
      dplyr::summarize('Pitch' = "Total",
                       ...
                       'FB %' = round(sum(TaggedHitType %in% c("FlyBall", "Popup"))/sum(PitchCall == "InPlay")*100, 1),
